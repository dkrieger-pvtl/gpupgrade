...
func TestStepStatus(t *testing.T) {
	...
	t.Run("when a step other than initialize is created its status is set to running", func(t *testing.T) {
		steps := []idl.Step{idl.Step_EXECUTE, idl.Step_FINALIZE, idl.Step_REVERT}
		for _, stepName := range steps {
			_, err := commanders.NewStep(stepName, &step.BufferedStreams{}, false)
			if err != nil {
				t.Errorf("unexpected err %#v", err)
			}

			status, err := store.Read(stepName)
			if err != nil {
	...
	t.Run("when a step other than finalize or revert completes its status is set to completed", func(t *testing.T) {
		steps := []idl.Step{idl.Step_INITIALIZE, idl.Step_EXECUTE}
		for _, stepName := range steps {
			st, err := commanders.NewStep(stepName, &step.BufferedStreams{}, false)
			if err != nil {
				t.Errorf("unexpected err %#v", err)
			}
