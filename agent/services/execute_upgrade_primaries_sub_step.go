...
func UpgradeSegments(sourceBinDir string, targetBinDir string, segments []Segment, stateDir string, checkOnly bool) error {
	...
	for _, segment := range segments {
		...
		go func() {
			...
			if err != nil {
				...
				agentErrs <- errors.Wrapf(err, "failed to upgrade%s primary on host %s with content %d", checkString, host, segment.Content)
			}
		}()
	}
