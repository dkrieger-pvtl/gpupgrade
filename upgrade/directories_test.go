...
func TestArchiveLogs(t *testing.T) {
	t.Run("fails to  archive a directory that is not contain expected logs", func(t *testing.T) {
		source := testutils.GetTempDir(t, "source")
		defer testutils.MustRemoveAll(t, source)
		...
		err := upgrade.ArchiveLogs(source, target)
		var merr *multierror.Error
		if !xerrors.As(err, &merr) {
			t.Fatalf("returned %#v want error type %T", err, merr)
		}
		...
		for _, err := range merr.Errors {
			expected := upgrade.ErrInvalidLogDirectory
			if !xerrors.Is(err, expected) {
				t.Errorf("returned error %#v want %#v", err, expected)
			}
		}
	...
	t.Run("successfully archives a directory that contains expected logs", func(t *testing.T) {
		source := testutils.GetTempDir(t, "source")
		defer testutils.MustRemoveAll(t, source)

		os.Create(filepath.Join(source, "gpupgrade_cli_1234.log"))
		os.Create(filepath.Join(source, "initialize_1234.log"))
		...
		if called != true {
			t.Errorf("exptected rename to be called")
		}
	})
}
