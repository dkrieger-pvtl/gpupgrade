...
func (s *Server) Revert(_ *idl.RevertRequest, stream idl.CliToHub_RevertServer) (err error) {
	...
	if s.UseLinkMode {
		// In any of the three cases of source cluster recovery in link-mode pg_upgrade, it is correct
		// to restore the pg_control file.  See the "Reverting to old cluster" section of
