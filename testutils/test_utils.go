...
func SetupDataDirs(t *testing.T) (source, target, tmpDir string) {
	...
	sourceMarker := upgrade.MarkerFileName(source, UpgradeID, idl.ClusterType_SOURCE)
	targetMarker := upgrade.MarkerFileName(target, UpgradeID, idl.ClusterType_TARGET)

	if err := utils.AddEmptyFileIdempotent(sourceMarker); err != nil {
		t.Errorf("unexpected err: %v", err)
	}
	if err := utils.AddEmptyFileIdempotent(targetMarker); err != nil {
		t.Errorf("unexpected err: %v", err)
	}
