...
function rename_rpm() {
    ...
    rpm=$(basename "rpm_gpupgrade_${release}"/*.rpm)
...
cp rpm_gpupgrade_oss/*.rpm build_artifacts_rc_oss/"$(rename_rpm oss)"
cp rpm_gpupgrade_enterprise/*.rpm build_artifacts_rc_enterprise/"$(rename_rpm enterprise)"
